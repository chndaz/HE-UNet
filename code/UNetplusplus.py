...
ecoph = 0
save_path = r'D:\python_new\ggb\runs'
now=time.time()
for i in tu:  # tu，tu_traget均为数据集
    ...
    img1 = torch.unsqueeze(img1, 0)
    model.load_state_dict(torch.load(r'D:\python_new\ggb\新图像_交稿件\各个模型保存的最优参数\UNetPLUSPLUS——2024-5-7_15-point_best_model.pth'))#这里的模型和小土堆的不同，保存的是模型的参数
    ouput = model(img1)
